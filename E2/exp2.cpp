...
int main() {
	...
	std::string f_img = "./E2/bgs-data/01.jpg";  // change the path.
	std::string f_bg = "./E2/bgs-data/01_bg.jpg";  // change the path.
	...
	bgSubstract(img.data, img_bg.data, width, height, img.step, channels, 100, mask.data, mask.step);
	imshow("fg mask", mask);
	waitKey(0);
